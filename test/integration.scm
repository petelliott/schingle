...
(define-test (schingle integration basic)
  (let-values (((r b)
    ...
    (assert (and (check-response r #:code 200 #:headers '((content-type text/plain)))
                 (equal? b "Hello World"))))
  (let-values (((r b)
  ...
  (let-values (((r b)
    ...
    (assert (and (check-response r #:code 500 #:headers '((content-type text/plain)))
...
(define-test (schingle integration return)
  (let-values (((r b)
    ...
    (assert (and (check-response r #:code 200 #:headers '((content-type application/json)))
                 (equal? b "{\"value\":\"blah\"}"))))
  (let-values (((r b)
                (handler (build-request (string->uri "http://localhost/xml/blah")) #f)))
    (assert (and (check-response r #:code 200 #:headers '((content-type application/xml)))
                 (equal? b "<value>blah</value>"))))
  (let-values (((r b)
                (handler (build-request (string->uri "http://localhost/html/blah")) #f)))
    (assert (and (check-response r #:code 200 #:headers '((content-type text/html)))
                 (equal? b "<html><p>blah</p></html>"))))
  (let-values (((r b)
                (handler (build-request (string->uri "http://localhost/sexp/blah")) #f)))
    (assert (and (check-response r #:code 200 #:headers '((content-type application/x.s-expression)))
                 (equal? b "((value . \"blah\"))"))))
  (let-values (((r b)
                (handler (build-request (string->uri "http://localhost/urlencoded/blah")) #f)))
    (assert (and (check-response r #:code 200 #:headers '((content-type application/x-www-form-urlencoded)))
                 (equal? b "value=blah")))))
...
(define-test (schingle integration static)
  (let-values (((r b)
    ...
    (assert (check-response r #:code 404 #:headers '((content-type text/plain)))))
  (let-values (((r b)
